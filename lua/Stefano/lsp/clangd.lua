return {
	...
	setup = function(opts)
		...
		-- augment capabilities with clangd-specific hints (keeps user-provided fields)
		caps.textDocument = caps.textDocument or {}
		caps.textDocument.completion = caps.textDocument.completion or {}
		caps.textDocument.completion.editsNearCursor = true
		caps.offsetEncoding = caps.offsetEncoding or { "utf-8", "utf-16" }
		...
		local function symbol_info(bufnr, client)
			...
			client:request(method_name, params, function(err, res)
				if err or not res or #res == 0 then
					-- clangd often returns an "error" here — ignore gracefully
					return
				end
		...
		local config = {
			...
			on_init = function(client, init_result)
				-- store offsetEncoding returned by clangd so other code can reference it
				if init_result and init_result.offsetEncoding then
					client.offset_encoding = init_result.offsetEncoding
				end
				-- also ensure client.offset_encoding field exists (compat)
				client.offset_encoding = client.offset_encoding
					or init_result and init_result.offsetEncoding
					or "utf-16"
			end,
